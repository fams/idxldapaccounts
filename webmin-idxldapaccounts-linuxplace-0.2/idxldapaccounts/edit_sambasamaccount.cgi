...
my $result =
  &LDAPSearch( $ldap, "(&(objectClass=inetOrgPerson)(objectClass=posixAccount)(uid=$user_uid))", $sattrs, $base );
if ( $result->count > 1 ) {
...
if ( $in{'delete'} ) {
	...
	if ( $userPassword =~ /^({Crypt}|{ssha}|{md5})/i ) {
		...
		print "<tr><td>"
		  . $text{'edit_sambaaccount_disable_nt'}
		  . "</td><td><input type=checkbox name=disableNT checked></td></tr>\n";
		print "<tr><td>"
		  . $text{'edit_sambaaccount_password'}
		  . "</td><td><input type=password name=userPassword></td></tr>\n";
		print "<tr><td>"
		  . $text{'edit_sambaaccount_retype_password'}
		  . "</td><td><input type=password name=retypeuserPassword></td></tr>\n";
	...
	print "<tr><td>" . $text{'edit_sambaaccount_server_home'} . "</td><td>";
	print "<select name=sambaHomeServer>\n";
	foreach ( split( /,/, $config{remotesmb} ) ) {
		my ( $host, $addr ) = split /:/;
		print "<option value='$_'>$host</option>";
	}
	print '</select>';
	print "</td></tr>\n";
...
sub sambaCreateUserArray {
	...
	foreach $k ( keys %in ) {
		next
		  if (
			$k =~ /^(cat|new|create|change|onglet|base|userPassword|retypeuserPassword|cryptedPassword|disableNT|sambaHomeServer)$/ );
		next if ( $in{$k} =~ /^\s*$/ );
		...
		if ( $k =~ /^(sambaProfilePath|sambaHomePath)$/ ) {
			my ( $host, $addr ) = split(/:/, $in{sambaHomeServer} );
			$attrs{$k} = $in{$k};
			$attrs{$k} =~ s/SERVER/$host/;
			next;
		}
		$attrs{$k} = $in{$k};
	}
...
sub sambaCreateUser {
	...
	foreach ( split( /,/, $config{remotesmb} ) ) {
        if ($in{sambaHomeServer} eq $_){
		    my ( $host, $addr ) = split /:/;
            $confighost=$addr;
        }
    }
	if ( !$smbserver->connect( 'execscript', $confighost ) ) {
		&error( $smbserver->{MSG} );
	}
